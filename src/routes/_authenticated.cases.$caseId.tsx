...
function CaseDetail() {
  ...
  if (!c)
    return (
      <Shell>
        <p className="text-sm text-muted-foreground">Case not found or no longer public.</p>
      </Shell>
    );
      ...
      <div className="mt-6 rounded-3xl border border-border bg-card p-8">
        ...
        <div className="mt-4 flex flex-wrap gap-4 text-sm text-muted-foreground">
          ...
          <span className="inline-flex items-center gap-1">
            ...
            {c.sessions_per_week} sessions / week · {c.session_length_minutes} min · {c.mode}
          </span>
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          ...
          <Detail
            label="Language"
            value={
              c.language_of_instruction === "zh-HK"
                ? "Cantonese"
                : c.language_of_instruction === "en"
                  ? "English"
                  : "Either"
            }
          />
          {c.schedule_note && <Detail label="Schedule" value={c.schedule_note} />}
          {c.student_grade_current && (
            <Detail label="Current grade" value={c.student_grade_current} />
          )}
        </div>
      </div>
      ...
      <div className="mt-8 rounded-3xl border border-border bg-card p-8">
        ...
        {myTutors.length === 0 ? (
          <div className="mt-4 rounded-2xl bg-muted/50 p-4 text-sm">
            ...
            <Link to="/join" className="font-semibold text-[color:var(--brand-teal)] underline">
              Apply to be a tutor
            </Link>{" "}
            first.
          </div>
          ...
          <div className="mt-4 space-y-4">
            <Select value={tutorId} onValueChange={setTutorId}>
              <SelectTrigger>
                <SelectValue placeholder="Choose tutor profile" />
              </SelectTrigger>
              <SelectContent className="max-h-64">
                {myTutors.map((t) => (
                  <SelectItem key={t.id} value={t.id}>
                    {t.display_name} ({t.tutor_code})
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
            <Textarea
              ...
              placeholder="Short note to the family (optional)"
            />
            <Button

...
type PublicCase = {
  ...
  budget_min: number | null; budget_max: number | null; urgency: string; language_of_instruction: string;
  description: string | null; created_at: string; status: string;
};
...
function CasesBoard() {
  ...
  const { data: cases = [], isLoading } = useQuery({
    ...
    queryFn: () => listFn({ data: { subject: subject === "all" ? null : subject, district: district === "all" ? null : district } }) as Promise<PublicCase[]>,
  });
    ...
    <div className="flex min-h-screen flex-col bg-background">
      ...
      <main className="flex-1">
        <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6">
          <div>
            <h1 className="text-3xl font-black tracking-tight text-[color:var(--brand-navy)] sm:text-4xl">Open tutoring cases</h1>
            <p className="mt-2 text-sm text-muted-foreground">Reviewed by our team. Tap a case to express interest.</p>
          </div>
          ...
          {isLoading ? (
            <p className="mt-10 text-sm text-muted-foreground">Loading cases…</p>
          ) : cases.length === 0 ? (
              ...
              {cases.map((c) => (
                ...
                >
                  ...
                  <div className="mt-auto flex flex-wrap gap-3 text-xs text-muted-foreground">
                    <span className="inline-flex items-center gap-1"><MapPin className="h-3 w-3" />{c.district ?? "Flexible"}</span>
                    <span className="inline-flex items-center gap-1"><Clock className="h-3 w-3" />{c.sessions_per_week}/wk · {c.mode}</span>
                    {(c.budget_min || c.budget_max) && <span className="inline-flex items-center gap-1"><Wallet className="h-3 w-3" />HK${c.budget_min ?? 0}–{c.budget_max ?? "?"}/hr</span>}
                  </div>
                </Link>
              ))}
            </div>

...
export function Hero08({
  ...
  const titleElement = title && (
    <h1 className={cn("text-foreground font-extrabold tracking-tight text-balance", vs.title)}>
  ...
  const socialProofElement =
    (socialProof || avatars?.length) && (socialProof || avatars?.length) !== 0 ? (
      <div className="flex flex-col items-start gap-3">
        {socialProof && <p className="text-foreground text-sm font-semibold">{socialProof}</p>}
        {avatars?.length ? (
          <div className="flex -space-x-2.5">
            {avatars.map((a) => (
              <Avatar key={a.src} className="ring-background size-9 ring-2">
  ...
  return (
    <section className={cn("bg-background relative isolate w-full overflow-hidden", className)}>
      <motion.div

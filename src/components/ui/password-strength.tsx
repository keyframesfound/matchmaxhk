...
const COMMON =
  /^(?:password|passw0rd|qwerty|letmein|welcome|admin|iloveyou|monkey|dragon|abc123|111111|123123|123456|12345678|123456789)/i;
const RUN = /(.)\1{3,}/;
...
export type PasswordStrengthCopy = {
  meterLabel: string;
  commonlyGuessed: string;
  srMet: string;
  srNotMet: string;
  announcedStrength: string;
  announcedGuessed: string;
  announcedAllMet: string;
  announcedStillNeeded: string;
};

export const defaultStrengthCopy: PasswordStrengthCopy = {
  meterLabel: "Password strength",
  commonlyGuessed: "Commonly guessed",
  srMet: "met",
  srNotMet: "not met",
  announcedStrength: "Password strength",
  announcedGuessed: "This is a commonly guessed pattern.",
  announcedAllMet: "All requirements met.",
  announcedStillNeeded: "Still needed",
};

export function makePasswordRules(labels: {
  length: string;
  case: string;
  digit: string;
}): PasswordRule[] {
  return [
    { id: "length", label: labels.length, test: (value) => value.length >= 8 },
    {
      id: "case",
      label: labels.case,
      test: (value) => /[a-z]/.test(value) && /[A-Z]/.test(value),
    },
    { id: "digit", label: labels.digit, test: (value) => /\d/.test(value) },
  ];
}

export const defaultPasswordRules: readonly PasswordRule[] = makePasswordRules({
  length: "8 characters or more",
  case: "Upper and lower case",
  digit: "A number",
});

const defaultLabels = ["Empty", "Weak", "Fair", "Strong"] as const;
  ...
  const state = useMemo(() => {
    ...
    const announcement =
      value.length === 0
        ? ""
        : [
            `${copy.announcedStrength} ${label.toLowerCase()}.`,
            guessable ? copy.announcedGuessed : "",
            unmet.length === 0
              ? copy.announcedAllMet
              : `${copy.announcedStillNeeded}: ${unmet.map((r) => r.label).join(", ")}.`,
          ]
            .filter(Boolean)
            .join(" ");
...
const TONES = {
  none: {
    bar: "bg-[color:var(--muted-foreground)]/30",
    text: "text-muted-foreground",
  },
  danger: {
    bar: "bg-[color:var(--destructive)]",
    text: "text-[color:var(--destructive)]",
  },
  caution: {
    bar: "bg-[color:var(--warning)]",
    text: "text-[#a16207] dark:text-[color:var(--warning)]",
  },
  safe: {
    bar: "bg-[color:var(--success)]",
    text: "text-[color-mix(in_oklab,var(--success)_72%,black)] dark:text-[color:var(--success)]",
  },
} as const;
...
export function PasswordStrength({
  ...
  const {
    ...
    guessable,
    announcement,
  } = usePasswordStrength(value, { rules, labels, copy, announceDelay });
    ...
    <div className={`w-full ${className}`}>
      ...
      >
        {Array.from({ length: max }, (_, i) => (
          <div
            key={i}
            className="relative h-1.5 overflow-hidden rounded-[2px] bg-[color:var(--muted)]"
          >
            <motion.span
      ...
      <div className="mt-2 flex h-5 items-center justify-between gap-3">
        ...
        <motion.span
          aria-hidden
          className="whitespace-nowrap text-xs leading-5 text-[#a16207] dark:text-[color:var(--warning)]"
          initial={false}
          animate={{ opacity: guessable ? 1 : 0 }}
          transition={reduced ? INSTANT : CROSSFADE}
        >
          {copy.commonlyGuessed}
        </motion.span>
      </div>
        ...
        <ul className="mt-3 grid gap-1.5">
          {evaluated.map((rule) => (
            <li key={rule.id} className="flex items-center gap-2">
              <span className="relative grid size-[14px] shrink-0 place-items-center rounded-[4px] border border-border text-white dark:border-white/20 dark:text-[color:var(--success-foreground)]">
                <motion.span
                  className="absolute inset-0 rounded-[3px] bg-[color:var(--success)]"
                  initial={false}
              ...
              <span className="sr-only">{rule.met ? copy.srMet : copy.srNotMet}</span>
            </li>
          ))}
        </ul>
      )}
